            ...
            if (address.startsWith(conf.sender.option.prefix)) {
                if (await checker.checkAddress(address) && await checker.checkIp(req.ip)) {
                    ...
                    sendTx(address).then(ret => {
                        console.log('sent OM to ', address)
...
async function sendTx(recipient) {
    ...
    console.log(await client.getSequence(firstAccount.address), "sequence");

    // const recipient = "cosmos1xv9tklw7d82sezh9haa573wufgy59vmwe6xxe5";
    const uom = conf.tx.uom;
    const fee = conf.tx.fee;
    await client.sendTokens(firstAccount.address, recipient, [conf.tx.uusdc], fee);
    console.log('sent USDC to ', address)

    await client.sendTokens(firstAccount.address, recipient, [conf.tx.uatom], fee);
    console.log('sent ATOM to ', address)
    return client.sendTokens(firstAccount.address, recipient, [uom], fee);
}
